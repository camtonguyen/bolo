...
{
  const result = resolveIssue(baseState, bulletin({ suspect: 'DR-4417', config: FLAGGED }), 0);
...
{
  const result = resolveIssue(baseState, bulletin({ config: CLEAN }), 0);
...
{
  const result = resolveIssue(baseState, bulletin({ config: IDENTIFIED }), 0);
...
{
  const result = resolveIssue(baseState, bulletin({ config: FLAGGED }), 0);
...
{
  const result = resolveIssue({ ...baseState, heat: 90 }, bulletin({ config: FLAGGED }), 0);
...
{
  const result = resolveIssue({ ...baseState, ending: 'burned' }, bulletin({ config: CLEAN }), 0);
...
// Complicit: a clean bulletin substituting another suspect's portrait frames them instead of ending normally.
{
  ...
  const result = resolveIssue(baseState, bulletin({ config: framedConfig }), 0);
...
{
  const first = resolveIssue(baseState, bulletin({ config: CLEAN }), 0);
  ...
  const reused = resolveIssue({ ...baseState, suspicion: { ...ZERO_SUSPICION, overlay: 4 } }, bulletin({ config: CLEAN }), 0);
  ...
  const actII = resolveIssue({ ...baseState, revealed: true, suspicion: suspicious }, bulletin({ config: CLEAN }), 0);
  assertEqual(actII.operatorFlagged, false, 'same edit and suspicion is not flagged before Internal Affairs is watching');
  const actIII = resolveIssue({ ...baseState, revealed: true, operatorFlagged: true, suspicion: suspicious }, bulletin({ config: CLEAN }), 0);
  assertEqual(shownVerdict(actIII).outcome, 'flagged', 'same edit and suspicion is flagged in Act III');
}

// liveDelta -- the editor's own crop/text/sticker tools -- reaches the verdict as tamper.
{
  const untouched = resolveIssue(baseState, bulletin({ config: CLEAN }), 0);
  assertEqual(untouched.operatorFlagged, false, 'no in-editor edits: the stamp alone stays under the tamper line');
  const edited = resolveIssue(baseState, bulletin({ config: CLEAN }), 0.5);
...
// Suspicion increments per technique used, regardless of whose case it is.
{
  ...
  const result = resolveIssue(baseState, bulletin({ suspect: 'DR-4417', config: gradedConfig }), 0);
...
// Bulletins prepend, newest first.
{
  ...
  const afterFirst = resolveIssue(baseState, first, 0);
  const second = bulletin({ suspect: 'DR-4417', controlNumber: 'LSP-BBB-002' as Bulletin['controlNumber'] });
  const afterSecond = resolveIssue({ ...baseState, bulletins: afterFirst.bulletins }, second, 0);
...
{
  const belowThreshold = resolveIssue({ ...baseState, casesClosed: CASES_BEFORE_REVEAL - 2 }, bulletin({ suspect: 'DR-4417' }), 0);
  ...
  const crossing = resolveIssue({ ...baseState, casesClosed: CASES_BEFORE_REVEAL - 1 }, bulletin({ suspect: 'DR-4417' }), 0);
  ...
  const alreadyRevealed = resolveIssue({ ...baseState, casesClosed: CASES_BEFORE_REVEAL, revealed: true }, bulletin({ suspect: 'DR-4417' }), 0);
